...
LocationReferences = Model(
    "LocationReferences",
    {
        ...
        "evse_uids": fields.List(
            CaseInsensitiveString(max_length=36),
            required=False,
...
Token = Model(
    "Token",
    {
        ...
        "group_id": CaseInsensitiveString(
            max_length=36,
            required=False,
        ...
        "default_profile_type": fields.String(
            enum=profile_type,
            required=False,
            description="The default Charging Preference. When this is provided, and a charging session is started on an Charge Point that support Preference base Smart Charging and support this ProfileType, the Charge Point can start using this ProfileType, without this having to be set via: Set Charging Preferences.",
        ),
        "energy_contract": fields.Nested(
            EnergyContract,
            required=False,
            description="When the Charge Point supports using your own energy supplier/contract at a Charge Point, information about the energy supplier/contract is needed so the CPO knows which energy supplier to use."
...
AuthorizationInfo = Model(
    "AuthorizationInfo",
    {
        ...
        "token": fields.Nested(
            Token,
            required=True,
            description="The complete Token object for which this authorization was requested.",
        ),
        "location": fields.Nested(
            LocationReferences,
            required=False,
            description="Optional reference to the location if it was included in the request, and if the EV driver is allowed to charge at that location. Only the EVSEs the EV driver is allowed to charge at are returned.",
        ),
