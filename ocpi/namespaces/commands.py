...
from ocpi.models.commands import (
    CancelReservation,
    CommandResponse,
    CommandResult,
    ReserveNow,
    ...
    StopSession,
    UnlockConnector,
    add_models_to_commands_namespace,
)
...
def sender():
    @commands_ns.route(
        "/START_SESSION",
        doc={"description": "OCPI Command API"},
    )
...
def receiver():
    ...
    class unlock_connector_result(Resource):
        ...
        def post(self):
            ...
            return make_response(
                self.command_manager.unlockConnectorResult, commands_ns.payload
            )

    @commands_ns.route(
        "/CANCEL_RESERVATION_RESULT",
        doc={"description": "OCPI Command API"},
    )
    @commands_ns.response(404, "Command not found")
    class cancel_reservation_result(Resource):
        ...
        def post(self):
            ...
            token = _check_access_token()
            return make_response(
                self.cm.cancelReservationResult, commands_ns.payload, token
            )
